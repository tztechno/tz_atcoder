...
fn check_by_day(ans: &Vec<Vec<Rect>>, d: usize) -> bool {
    let da = &ans[d];
    for i1 in 0..da.len() {
        let a1 = da[i1];
        if a1.area() <= 0 {
...
fn check(ans: &Vec<Vec<Rect>>) -> bool {
    // #[cfg(feature = "test")]
    {
        for d in 0..ans.len() {
            if !check_by_day(ans, d) {
                return false;
            }
        }
    }
...
// 幅と高さだけ保持する構造体、どこに配置するかはまた別の場所で定義する
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
struct OrderRectWithInd {
    width: i32,
    height: i32,
    ind: usize,
}
impl OrderRectWithInd {
    #[inline]
    fn area(&self) -> i32 {
        self.width * self.height
    }
}
type RectsWithInd = Vec<OrderRectWithInd>;

#[derive(Clone)]
struct RectManager {
    columns: Vec<Vec<RectsWithInd>>,
    columns_want_area_sum: Vec<Vec<i32>>, // sum[d][order] : d 日目の、order 番目の列でほしい面積の合計
    indexes: Vec<Vec<usize>>, // indexes[d] : d日目において、有効な columns のインデックスの vec
    w_sum: Vec<i32>,          // w_sum[day]: day 日目の w の合計
    ans: Vec<Vec<Rect>>,
    input: Input,
}
impl RectManager {
    fn new(input: Input, columns: Vec<Vec<RectsWithInd>>) -> Self {
        let mut columns_want_area_sum = vec![vec![0; input.n as usize]; input.d as usize];
        let mut w_sum = vec![0; input.d as usize];

        for d in 0..input.d as usize {
            for n in 0..input.n as usize {
                // 初期値はコレでいい
                w_sum[d] += columns[d][n][0].width;

                columns_want_area_sum[d][n] = columns[d][n]
                    .iter()
                    .map(|rect| input.a[d][rect.ind])
                    .sum::<i32>();
            }
        }

        let mut indexes = vec![Vec::with_capacity(input.n as usize); input.d as usize];
        for d in 0..input.d as usize {
            for n in 0..input.n as usize {
                indexes[d].push(n);
            }
        ...
        Self {
            columns,
            indexes,
            columns_want_area_sum,
            w_sum,
            ans: vec![vec![]; input.d as usize],
            input,
        }
    }

    #[inline]
    fn set_by_order(&mut self, d: usize, order: usize, rects: RectsWithInd) {
        self.columns[d][order] = rects;
    }

    #[inline]
    fn get_by_order(&self, d: usize, order: usize) -> &RectsWithInd {
        &self.columns[d][order]
    }

    #[inline]
    fn swap_by_order(&mut self, d: usize, order1: usize, order2: usize) {
        self.columns[d].swap(order1, order2);
    }

    // d 日目において、from の上から from_order 番目の矩形を to 番目の上から to_order に移動させる
    // false 返したときは移動してないことにする、
    fn move_rect(
        &mut self,
        d: usize,
        from: usize,
        ...
        let from_rects = self.get_by_order(d, from).clone();
        let to_rects = self.get_by_order(d, to).clone();

        // to 側 w の調整
        let mut to_want_area_sum = 0;
        to_want_area_sum += self.input.a[d][from_rects[from_order].ind];
        for rect in to_rects.iter() {
            to_want_area_sum += self.input.a[d][rect.ind];
        }

        let mut new_to_w = (to_want_area_sum + 1000 - 1) / 1000;
        loop {
            if new_to_w > 500 {
                return false;
            }
            ...
            let h = (self.input.a[d][from_rects[from_order].ind] + new_to_w - 1) / new_to_w;
            h_sum += h;
            real_area += h * new_to_w;

            for rect in to_rects.iter() {
                let h = (self.input.a[d][rect.ind] + new_to_w - 1) / new_to_w;
                h_sum += h;
                real_area += h * new_to_w;
            }

            if h_sum <= 1000 && real_area >= to_want_area_sum {
                break;
            }
        ...
        // from 側の w の調整
        let mut from_want_area_sum = 0;
        for order in 0..from_rects.len() {
            if order == from_order {
                continue;
            }
            let rect = from_rects[order];
            from_want_area_sum += self.input.a[d][rect.ind];
        }

        let mut new_from_w = (from_want_area_sum + 1000 - 1) / 1000;
        loop {
            // 汚いコード, new_from_w == 0 であることを後ろで使ってる
            if new_from_w == 0 {
                break;
            }
            if new_from_w > 500 { // 1000 はいらんやろ
                return false;
            }
            ...
            for order in 0..from_rects.len() {
                ...
                let rect = from_rects[order];
                let h = (self.input.a[d][rect.ind] + new_from_w - 1) / new_from_w;
                h_sum += h;
                real_area +=
                    ((self.input.a[d][rect.ind] + new_from_w - 1) / new_from_w) * new_from_w;
            }

            if h_sum <= 1000 && real_area >= from_want_area_sum {
                break;
            }
        ...
        // w の判定,
        // TODO: 上手いことやってcheck処理がなくなるような w を設定したい
        // 初期値が w_sum[d] > 1000 なので割と処理がむずい、変にやると改善が進まん

        let mut new_from_rects = Vec::new();
        for order in 0..from_rects.len() {
            if order == from_order {
                continue;
            }
            new_from_rects.push(OrderRectWithInd {
                ind: from_rects[order].ind,
                width: new_from_w,
                height: (self.input.a[d][from_rects[order].ind] + new_from_w - 1) / new_from_w,
            });
        }
        // 何もなくなったとき
        if new_from_rects.len() == 0 {
            self.indexes[d].retain(|&x| x != from);
        }

        self.set_by_order(d, from, new_from_rects);

        let mut new_to_rects = Vec::new();
        for i in 0..to_rects.len() {
            new_to_rects.push(OrderRectWithInd {
                ind: to_rects[i].ind,
                width: new_to_w,
                height: (self.input.a[d][to_rects[i].ind] + new_to_w - 1) / new_to_w,
            });
        }
        new_to_rects.insert(
            to_order,
            OrderRectWithInd {
                ind: from_rects[from_order].ind,
                width: new_to_w,
                height: (self.input.a[d][from_rects[from_order].ind] + new_to_w - 1) / new_to_w,
            },
        );

        // 新しく出来た
        if to_rects.len() == 0 {
            self.indexes[d].push(to);
        }

        self.set_by_order(d, to, new_to_rects);

        true
    }

    // d 日目において、from の上から rect_order 番目の矩形と to の上から to_order 番目の矩形を交換する
    fn swap_rect(
        &mut self,
        d: usize,
        from: usize,
        ...
        let mut to_want_area_sum = 0;
        to_want_area_sum += self.input.a[d][from_rects[from_order].ind];
        for order in 0..to_rects.len() {
            if order == to_order {
                continue;
            }
            let rect = to_rects[order];
            to_want_area_sum += self.input.a[d][rect.ind];
        }

        let mut to_w = (to_want_area_sum + 1000 - 1) / 1000;
        loop {
            if to_w > 1000 {
                return false;
            }
            ...
            let h = (self.input.a[d][from_rects[from_order].ind] + to_w - 1) / to_w;
            h_sum += h;
            real_area += h * to_w;
            ...
            for order in 0..to_rects.len() {
                ...
                let rect = to_rects[order];
                let h = (self.input.a[d][rect.ind] + to_w - 1) / to_w;
                h_sum += h;
                real_area += h * to_w;
            }
        ...
        // from 側の w の調整
        let mut from_want_area_sum = 0;
        from_want_area_sum += self.input.a[d][to_rects[to_order].ind];
        for order in 0..from_rects.len() {
            if order == from_order {
                continue;
            }
        ...
        let mut from_w = (from_want_area_sum + 1000 - 1) / 1000;
        loop {
            if from_w > 1000 {
                return false;
            }
            ...
            let h = (self.input.a[d][to_rects[to_order].ind] + from_w - 1) / from_w;
            h_sum += h;
            real_area += h * from_w;
            ...
            for order in 0..from_rects.len() {
                ...
                let rect = from_rects[order];
                let h = (self.input.a[d][rect.ind] + from_w - 1) / from_w;
                h_sum += h;
                real_area += ((self.input.a[d][rect.ind] + from_w - 1) / from_w) * from_w;
            }
        ...
        for order in 0..from_rects.len() {
            ...
            new_from_rects.push(OrderRectWithInd {
                ind: from_rects[order].ind,
                width: from_w,
                height: (self.input.a[d][from_rects[order].ind] + from_w - 1) / from_w,
            });
        }
        new_from_rects.insert(
            from_order,
            OrderRectWithInd {
                ind: to_rects[to_order].ind,
                width: from_w,
                height: (self.input.a[d][to_rects[to_order].ind] + from_w - 1) / from_w,
            },
        );
        ...
        for order in 0..to_rects.len() {
            ...
            new_to_rects.push(OrderRectWithInd {
                ind: to_rects[order].ind,
                width: to_w,
                height: (self.input.a[d][to_rects[order].ind] + to_w - 1) / to_w,
            });
        }
        new_to_rects.insert(
            to_order,
            OrderRectWithInd {
                ind: from_rects[from_order].ind,
                width: to_w,
                height: (self.input.a[d][from_rects[from_order].ind] + to_w - 1) / to_w,
            },
        );
  ...
  // d 日目において、from の上から rect_order 番目の矩形と to の上から to_order 番目の矩形を交換する
    fn swap_rect_same(
        ...
        self.columns[d][n].swap(from_order, to_order);

        true
    }

    fn update_ans_by_day(&mut self, d: usize) {
        self.ans[d] = vec![Rect::default(); self.input.n as usize];

        let mut x1 = 0;

        self.indexes[d].sort_unstable();
        let last_order = self.indexes[d].iter().max().unwrap();

        for order in self.indexes[d].iter() {
            let rects = &self.columns[d][*order];
            let mut y1 = 0;
            for i in 0..rects.len() {
                let rect = rects[i];
                self.ans[d][rect.ind] = Rect {
                    x1: x1,
                    y1: y1,
                    x2: (x1 + rect.width).min(1000),
                    y2: y1 + rect.height,
                };

                // TODO: 要調査、スペースが大きいとき&横線が多い時は伸ばさないほうが良い
                if order == last_order {
                    self.ans[d][rect.ind].x2 = 1000;
                }
                if i == rects.len() - 1 {
                    self.ans[d][rect.ind].y2 = 1000;
                }
    ...
    fn init_ans(&mut self) {
        for d in 0..self.columns.len() {
            self.update_ans_by_day(d);
        }
    }
}
...
impl Line {
    ...
    // 重なってない距離を出す
    #[inline]
    fn non_overlapping_distance(&self, other: Line) -> i32 {
        let over = max(0, min(self.right, other.right) - max(self.left, other.left));
        self.distance() + other.distance() - 2 * over
    }

    #[inline]
    fn overlapping_distance(&self, other: Line) -> i32 {
        max(0, min(self.right, other.right) - max(self.left, other.left))
    }
}

struct CostManager {
    tate: Vec<HashSet<i32>>,            // d 日目の x 座標の縦線を持つ集合
    yoko: Vec<HashMap<i32, HashSet<Line>>>, // d 日目の座標 y の横線の集合, yoko[d][y] = d 日目の座標 y にある、横線の集合
    area_cost: Vec<i32>,                // d 日目のコスト
    tate_cost: Vec<i32>,                // d 日目のコスト
    yoko_cost: Vec<i32>,                // d 日目のコスト
}

impl CostManager {
    fn new() -> CostManager {
        CostManager {
            tate: vec![HashSet::new(); 50],
            yoko: vec![HashMap::new(); 50],
            area_cost: vec![0; 50],
            tate_cost: vec![0; 50],
            yoko_cost: vec![0; 50],
        }
    ...
    // TODO: スコア関数の修正, 横線とか全く計算してない
    fn init(&mut self, input: &Input, ans: &Vec<Vec<Rect>>) -> i32 {
        self.tate = vec![HashSet::new(); 50];

        if !check(&ans) {
            // try_fold が Err を返した場合、早期リターン
            return i32::MAX;
        }
        ...
        for n in 0..input.n as usize {
            ...
            if rec.x1 != 0 {
                self.tate[0].insert(rec.x1);
            }
            if rec.x2 != input.w {
                self.tate[0].insert(rec.x2);
            }

            if rec.y1 != 0 {
                self.yoko[0].entry(rec.y1).or_insert(HashSet::new()).insert(Line {
                    left: rec.x1,
                    right: rec.x2,
                });
            }
            if rec.y2 != input.w {
                self.yoko[0].entry(rec.y2).or_insert(HashSet::new()).insert(Line {
                    left: rec.x1,
                    right: rec.x2,
                });
            }
        ...
        for n in 0..input.n as usize {
            self.area_cost[0] += max(0, 100 * (input.a[0][n] - ans[0][n].area()));
        }

        for d in 1..input.d as usize {
            for n in 0..input.n as usize {
                let rec = ans[d][n];

                if rec.x1 != 0 {
                    self.tate[d].insert(rec.x1);
                }
                if rec.x2 != input.w {
                    self.tate[d].insert(rec.x2);
                }

                if rec.y1 != 0 {
                    self.yoko[d].entry(rec.y1).or_insert(HashSet::new()).insert(Line {
                        left: rec.x1,
                        right: rec.x2,
                    });
                }
                if rec.y2 != input.w {
                    self.yoko[d].entry(rec.y2).or_insert(HashSet::new()).insert(Line {
                        left: rec.x1,
                        right: rec.x2,
                    });
                }
            ...
            self.tate_cost[d] += 1000 * self.tate[d].difference(&self.tate[d - 1]).count() as i32;
            self.tate_cost[d] += 1000 * self.tate[d - 1].difference(&self.tate[d]).count() as i32;

            for (y, lines1) in self.yoko[d].iter() {
                // d-1日目にyがなかったらそのまま足す
                if !self.yoko[d - 1].contains_key(y) {
                    self.yoko_cost[d] += lines1.iter().map(|line| line.distance()).sum::<i32>();
                    continue;
                }

                for line2 in self.yoko[d - 1].get(y).unwrap().iter() {
                    self.yoko_cost[d] += lines1
                        .iter()
                        .map(|line| line.non_overlapping_distance(*line2))
                        .sum::<i32>();
                }
            }

            for (y, lines) in self.yoko[d-1].iter() {
                if !self.yoko[d].contains_key(y) {
                    self.yoko_cost[d] += lines.iter().map(|line| line.distance()).sum::<i32>();
                    continue;
                }
            }

            for n in 0..input.n as usize {
                self.area_cost[d] += max(0, 100 * (input.a[d][n] - ans[d][n].area()));
            }
        }

        self.area_cost.iter().sum::<i32>() + self.tate_cost.iter().sum::<i32>() + self.yoko_cost.iter().sum::<i32>()
    }

    // 左から表示する想定, こ
    // TODO: スコア関数の高速化、0.01 秒ぐらいかかる
    fn calc_score(&mut self, input: &Input, ans: &Vec<Vec<Rect>>, d: usize) -> i32 {
        if !check_by_day(&ans, d) {
            return i32::MAX;
        }
        ...
        self.tate_cost[d] = 0;
        self.yoko_cost[d] = 0;
        self.yoko[d] = HashMap::new();
        self.tate[d] = HashSet::new();
        for n in 0..input.n as usize {
            let rec = ans[d][n];
        ...
        // d+1 の更新
        if d != input.d as usize - 1 {
            self.tate_cost[d + 1] = 0;

            self.tate_cost[d + 1] +=
                1000 * self.tate[d + 1].difference(&self.tate[d]).count() as i32;
            self.tate_cost[d + 1] +=
                1000 * self.tate[d].difference(&self.tate[d + 1]).count() as i32;
        }

        self.tate_cost.iter().sum::<i32>() + self.area_cost.iter().sum::<i32>() + self.yoko_cost.iter().sum::<i32>()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_rect_score() {
        let mut cost_manager = CostManager::new();
        let input = Input {
            w: 1000,
            d: 2,
            n: 1,
        ...
        assert_eq!(cost_manager.init(&input, &out), 1000);
    }
}
...
fn solve(input: &Input) -> Vec<Vec<Rect>> {
    let mut columns = vec![vec![RectsWithInd::default(); input.n as usize]; input.d as usize];

    #[cfg(feature = "test")]
    let mut records_map: HashMap<String, Vec<Record>> = HashMap::new();
    let mut timers_map: HashMap<String, Vec<f64>> = HashMap::new();

    // TODO:初期値をもっといい感じにしたい、極値解にハマる可能性有り、2.9で収束してるし今は良いかも
    ...
    for d in 0..input.d as usize {
        ...
        for n in 0..input.n as usize {
            ...
            let use_x = (a + 1000 - 1) / 1000;

            columns[d][n] = vec![OrderRectWithInd {
                ind: n,
                width: use_x,
                height: 1000,
            }];
    ...
    let mut current_rect_manager = RectManager::new(input.clone(), columns);
    current_rect_manager.init_ans();

    let mut best_ans = current_rect_manager.ans.clone();
    let mut cost_manager = CostManager::new();
    let mut current_cost = cost_manager.init(&input, &current_rect_manager.ans);
    let mut best_cost = current_cost;

    let mut rng = Pcg64Mcg::new(24358);

    const TEMP0: f64 = 1e-5;
    const TEMP1: f64 = 1e-7;
    const TL: f64 = 2.96;

    let mut t = 0.0;
    let mut temp = TEMP0.powf(1.0 - t / TL) * TEMP1.powf(t / TL);
    ...
    loop {
        if loop_cnt % 100 == 0 {
            t = get_time();
            temp = TEMP0.powf(1.0 - t / TL) * TEMP1.powf(t / TL);
        }
        ...
        match r {
            0 => {
                let d = rng.gen_range(0..input.d as usize);

                if current_rect_manager.indexes[d].len() == 1 {
                    continue;
                }

                let i1 = rng.gen_range(0..(current_rect_manager.indexes[d].len()-1));
                let n1 = current_rect_manager.indexes[d][i1];
                let n2 = current_rect_manager.indexes[d][i1+1];

                if n1 == n2 {
                    continue;
                }

                current_rect_manager.swap_by_order(d, n1, n2);
                current_rect_manager.update_ans_by_day(d);

                let cost = cost_manager.calc_score(&input, &current_rect_manager.ans, d);
                ...
                if current_cost > cost {
                    ...
                    if current_cost < best_cost {
                        ...
                        best_ans = current_rect_manager.ans.clone();
                    }
                    ...
                    current_rect_manager.swap_by_order(d, n1, n2);
                    current_rect_manager.update_ans_by_day(d);
                    cost_manager.calc_score(&input, &current_rect_manager.ans, d);
                }
            }
            1 => {
                let d = rng.gen_range(0..input.d as usize);
                ...
                let from = current_rect_manager.indexes[d]
                    [rng.gen_range(0..current_rect_manager.indexes[d].len())];
                let to = current_rect_manager.indexes[d]
                    [rng.gen_range(0..current_rect_manager.indexes[d].len())];
                ...
                let from_order = rng.gen_range(0..current_rect_manager.get_by_order(d, from).len());
                let to_order = rng.gen_range(0..=current_rect_manager.get_by_order(d, to).len());

                let m = current_rect_manager.move_rect(d, from, from_order, to, to_order);
                ...
                let cost = cost_manager.calc_score(&input, &current_rect_manager.ans, d);
                timers_map.entry("calc_score".to_string()).or_insert(vec![]).push(
                    start.elapsed().as_secs_f64()
                );
                let cost_diff = (current_cost - cost) as f64;

                if current_cost > cost || rng.gen_bool(f64::exp(cost_diff / temp)) {
                    if current_cost <= cost {
                        yaki_cnt += 1;
                    }
                    current_cost = cost;
                    ...
                    current_rect_manager.move_rect(d, to, to_order, from, from_order);
                    current_rect_manager.update_ans_by_day(d);
                    cost_manager.calc_score(&input, &current_rect_manager.ans, d);
                }
            }
            2 => {
                ...
                if current_rect_manager.indexes[d].len() == 0 {
                    continue;
                }
                ...
                let m = current_rect_manager.swap_rect(d, from, from_order, to, to_order);
                if !m {
                    continue;
                }
                ...
                if current_cost > cost || rng.gen_bool(f64::exp(cost_diff / temp)) {
                    ...
                    current_rect_manager.swap_rect(d, to, to_order, from, from_order);
                    current_rect_manager.update_ans_by_day(d);
                    cost_manager.calc_score(&input, &current_rect_manager.ans, d);
                }
            }
            3 => {
                let d = rng.gen_range(0..input.d as usize);
                let n = current_rect_manager.indexes[d]
                    [rng.gen_range(0..current_rect_manager.indexes[d].len())];
                if current_rect_manager.get_by_order(d, n).len() == 1 {
                    continue;
                }

                let from_order = rng.gen_range(0..(current_rect_manager.get_by_order(d, n).len()-1));
                let to_order = from_order + 1;

                if !current_rect_manager.swap_rect_same(d, n, from_order, to_order) {
                    continue;
                }
                ...
                if current_cost > cost {
                    ...
                    current_rect_manager.swap_rect_same(d, n, from_order, to_order);
                    current_rect_manager.update_ans_by_day(d);
                    cost_manager.calc_score(&input, &current_rect_manager.ans, d);
                }
            }
    ...
    #[cfg(feature = "test")]
    {
        ...
        eprintln!("yaki_cnt: {:?}", yaki_cnt);
        for (k, records) in records_map.iter() {
            eprintln!("update{} cnt: {:?}", k, records.len());
            let mut wtr = WriterBuilder::new()
                .from_path(format!("sample_{}_.csv", k))
                .unwrap();
        ...
        for (k, times) in timers_map.iter() {
            eprintln!(
                "{} time: {:?}",
                k,
                times.iter().sum::<f64>() * 1000.0 / times.len() as f64
            );
        }
    }
...
fn main() {
    ...
    #[cfg(feature = "test")]
    {
        ...
        for d in 0..input.d as usize {
            ...
            for n in 0..input.n as usize {
                sum += input.a[d][n];
                amari += max(0, ans[d][n].area() - input.a[d][n]);
                loss += max(0, input.a[d][n] - ans[d][n].area());
            }
            loss_sum += loss;
            ...
            if loss != 0 {
                ...
                eprintln!("sum: {}", sum);
                eprintln!("loss: {}", loss);
                eprintln!("amari: {}", amari);
            }
        }

        if loss_sum != 0 {
            panic!("loss_sum: {}", loss_sum);
        }
    }
}
