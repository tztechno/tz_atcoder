...
# 最初に o の数を数える
fixed_o_count = S.count('o')

# o がすでに K 個なら、残りの ? は全て . にすればいい
if fixed_o_count == K:
    print(''.join('.' if c == '?' else c for c in S))
    exit()

# o が多すぎる or 連続o すでにあるなら不可能
if fixed_o_count > K:
    print(''.join('.' if c == '?' else c for c in S))
    exit()
...
for i in range(N):
    if S[i] == 'o' or S[i] == '.':
        ...
        if must_o[i] and not must_dot[i]:
            res.append('o')
        elif must_dot[i] and not must_o[i]:
            res.append('.')
        else:
            res.append('?')
